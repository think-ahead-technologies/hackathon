...
# ABOUTME: Real ethos-u-vela 5.1 column names differ from the gate's; this is the ML->Platform handoff.
import csv
import os
import re
import shutil
...
PIPELINE_BUILD = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    "dashboard", "pipeline", "build")
...
def main():
    ...
    os.makedirs(PIPELINE_BUILD, exist_ok=True)
    summary_out = os.path.join(PIPELINE_BUILD, "vela-summary.csv")
    with open(summary_out, "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=list(norm.keys()))
        w.writeheader()
        w.writerow(norm)
    shutil.copy(os.path.join(BUILD, "model_int8_vela.tflite"),
                os.path.join(PIPELINE_BUILD, "model_int8_vela.tflite"))

    print(f"CPU ops {cpu_ops} / NPU ops {npu_ops}  ->  fallback "
          f"{cpu_ops / (cpu_ops + npu_ops) * 100:.1f}%")
    print(f"normalized summary -> {summary_out}")
    print(f"vela artifact      -> {os.path.join(PIPELINE_BUILD, 'model_int8_vela.tflite')}")
